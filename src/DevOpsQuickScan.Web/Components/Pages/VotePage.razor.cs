...
public partial class VotePage : ComponentBase
{
     ...
     protected override async Task OnInitializedAsync()
     {
          ...
          _hubConnection.On<Question>("ReceiveQuestion", async (question) =>
          {
               await InvokeAsync(() =>
               {
          ...
          await _hubConnection.InvokeAsync("JoinSession", SessionId, "Anonymous");
     }
     ...
     private async Task SubmitVote()
     {
          ...
          {
               var vote = new Vote(_currentQuestion.Id, _selectedAnswerId);

               await _hubConnection.InvokeAsync("SubmitVote", SessionId, vote);
               _isAnswerSelected = false;
          }
     }
     // [Parameter]
